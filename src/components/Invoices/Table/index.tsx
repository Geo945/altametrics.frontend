...
function EnhancedTableHead(props: EnhancedTableProps) {
    ...
    const createSortHandler =
        (property: keyof IInvoice) => (event: React.MouseEvent<unknown>) => {
...
const InvoicesTable = ({invoices}: TInvoicesTableProps)=> {
    ...
    const [orderBy, setOrderBy] = React.useState<keyof IInvoice>('due_date');
    ...
    const handleOpenModal = (event: React.MouseEvent<unknown>, id: string) => {
        setOpenModal(true)
        setModalData(invoices.find((invoice) => invoice.id === id) as IInvoice)
    }
    ...
    return (
        <Box sx={{ width: '100%' }}>
            ...
            <FormControlLabel
                ...
                label="Dense padding"
            />
            <ModalPopUp submitBtnLabel="Submit" open={openModal} handleClose={() => setOpenModal(false)} title="Invoice info" handleSubmit={() => setOpenModal(false)}>
                <Box display="grid" gap={2}>
                    <Typography>Due_date: {modalData?.due_date}</Typography>
                    <Typography>Amount: {modalData?.amount}</Typography>
                    <Typography>Payee: {modalData?.name}</Typography>
